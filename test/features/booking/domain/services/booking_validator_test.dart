...
class FakeTimeProvider implements TimeProvider {
  FakeTimeProvider(this.currentTime);
...
void main() {
  final timeProvider = FakeTimeProvider(DateTime(2026, 9, 24, 8));

  final validator = BookingValidator(timeProvider: timeProvider);
  ...
  group('Past date and time validation', () {
    test('rejects booking on a past date', () {
      ...
      expect(result.failureReason, BookingFailureReason.pastDate);
    });
    ...
    test('rejects a slot that already started today', () {
      ...
      expect(result.isValid, isFalse);
      expect(result.failureReason, BookingFailureReason.pastTime);
    });

    test('rejects a slot starting exactly at current time', () {
      final slots = fullDay();
    ...
    test('valid start times exclude times that already passed today', () {
      final slots = fullDay();
